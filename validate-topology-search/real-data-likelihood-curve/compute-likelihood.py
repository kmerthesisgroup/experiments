...
def get_datasets(dataset_dir):
    ...
    species_files = [ re.sub(r'-\d+-\d+-tree.txt$', '-species.txt', tree_file) for tree_file in tree_files ]
...
def main():
    ...
    for tree_file, species_file in datasets:
        process_tree(f'{DATASET_DIR}/{tree_file}', f'{INPUT_TOPOLOGY_DIR}/{species_file}')
