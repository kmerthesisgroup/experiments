...
def main():
  ...
  for file in files:
    if file.endswith('-tree.txt'):
      original_treefile = os.path.join(INPUT_TOPOLOGY_DIRECTORY, file)
      original_tree = pt.load_from_file(original_treefile, only_leaves=True)
      ...
      outfile_prefix = re.sub(r'-tree\.txt$', '-', file)
      outfile_prefix = os.path.join(OUTPUT_DIRECTORY, outfile_prefix)
